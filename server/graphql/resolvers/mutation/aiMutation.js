...
module.exports = {
  ...
  shaleAPI: async (parent, args, context, info) => {
    const { textForShale } = args.fields;

    if (!textForShale) {
      new ApolloError("The textForShale is required");
    }

    try {
      res = await useShaleAPI(textForShale, (temperature = 0));
      console.log("res+++++++++", res);
      return {
        outputForShale: res.text,
      };
    } catch (err) {
